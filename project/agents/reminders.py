...
The brief allows a manual "check reminders" button as the proof of
concept for Tier 3 — no real background scheduler. We provide:
...
def due_within(user_id: int, days: int = 7, today: Optional[dtdate] = None) -> list[dict]:
    ...
    for row in list_for_user(user_id):
        ...
        if today <= target <= horizon:
            out.append({**row, "_target_date": target.isoformat()})
    return out
